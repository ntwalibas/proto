...
std::unique_ptr<Expression>
Parser::parseTernaryIfExpression()
{
    std::unique_ptr<Expression> condition = parseLogicalOrExpression();

    if (match(PROTO_QUESTION)) {
        Token op_token = peekBack();
        ...
        std::unique_ptr<Expression> rvalue = parseTernaryIfExpression();
        std::unique_ptr<Expression> ternif_expr =
            std::make_unique<TernaryIfExpression>(
                op_token,
                std::move(condition),
                std::move(lvalue),
                std::move(rvalue)
        ...
        condition = std::move(ternif_expr);
    }
...
Parser::parseUnaryExpression()
{
    if (
        match(PROTO_PLUS)           ||
        match(PROTO_MINUS)          ||
        match(PROTO_BITWISE_NOT)    ||
        match(PROTO_LOGICAL_NOT)
    ) {
        Token op_token = peekBack();
        std::unique_ptr<Expression> rec_expr = parseUnaryExpression();
        enum UnaryType un_type;
        switch (op_token.type) {
            case PROTO_PLUS:
                un_type = UnaryType::Plus;
                break;
            case PROTO_MINUS:
                un_type = UnaryType::Minus;
                break;
            case PROTO_BITWISE_NOT:
                un_type = UnaryType::BitwiseNot;
                break;
            case PROTO_LOGICAL_NOT:
                un_type = UnaryType::LogicalNot;
                break;
            default:;
        }
